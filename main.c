...
static uint32_t amin = 0;
static uint32_t w = 1820;
static uint32_t h = 1024;
static uint32_t t_show = 0;
static uint32_t n_show = 0;
static uint32_t size = 2048;
...
static size_t pal_max = sizeof(pal) / sizeof(pal[0]) - 1;
...
void show(SDL_Surface *s)
{
	...
	for(i=0; i<w * h; i++) {
		uint8_t c = (*p) >> 24;
		if(c > 30) c = c - 2;
		uint32_t color = pal[c * pal_max / 255];
		*p++ = (c << 24) | color;
	}
...
void draw(uint32_t a, uint32_t s, uint32_t color)
{
	if(amin == 0) amin = a;
	a = (a - amin);
	...
	while(s--) {
		...
		x *= (w / (double)size);
		y *= (h / (double)size);
		if(a < size * size && x > 0 && y > 0 && x < w && y < h) {
			uint32_t *p = (uint32_t *)screen->pixels + y * w + x;
			*p = color;
		}
		a++;
	}
...
void do_malloc(uint32_t a, uint32_t s)
{
	draw(a, s, 0xff000000);
}


void do_free(uint32_t a, uint32_t s)
{
	draw(a, s, 0x00000000);
}
...
int main(int argc, char **argv)
{
	...
	for(;;) {
		uint32_t a1, a2, so, sn;
		uint32_t r = scanf("0x%x 0x%x %d %d\n", &a1, &a2, &so, &sn);
		if(r != 4) {
			break;
		}
